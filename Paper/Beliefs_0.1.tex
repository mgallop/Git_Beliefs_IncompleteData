...
%Can't get Math formatting to work
%\begin{equation}
%\begin{array}
%log\frac{Pr(\text{True Category is 1}|x_i,w_i)}{Pr(\text{True Category is $x_{i}}|x_i,w_i$)}=a_10+\sum_{j=1}^J a_{1j}  w_i^j \\
%log\frac{Pr(\text{True Category is 2}|x_i,w_i)}{Pr(\text{True Category is $x_{i}}|x_i,w_i$)}=a_20+\sum_{j=1}^J a_{2j}  w_i^j \\
%\vdots \\
%log\frac{Pr(\text{True Category is $(k-1)}|x_i,w_i$)}{Pr(\text{True Category is $x_{i}$}$|x_i,w_i)$}=a_{(k-1)0}+\sum_{j=1}^J %a_{(k-1)j}  w_i^j
%\end{array}
%\end{equation}
